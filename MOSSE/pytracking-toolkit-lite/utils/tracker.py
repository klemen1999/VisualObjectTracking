...
from utils.io_utils import save_regions, save_vector, save_vector_time
...
class Tracker(ABC):
    ...
    def evaluate(self, dataset: Dataset, results_dir: str):
        ...
        for sequence in dataset.sequences:
            ...
            # file with times per frame and information if frame was init or not (0=init frame, 1=track frame)
            time_type_path = os.path.join(sequence_results_dir, '%s_%03d_time_type.txt' % (sequence.name, 1))
            ...
            while frame_index < sequence.length:
                ...
                if frame_index == init_frame:
                    ...
                    time_needed = timer() - t_
                    times[frame_index] = time_needed
                    times_type[frame_index] = (time_needed, 0)
                    ...
                    time_needed = timer() - t_
                    times[frame_index] = time_needed
                    times_type[frame_index] = (time_needed, 1)
            ...
            save_vector_time(times_type, time_type_path)
