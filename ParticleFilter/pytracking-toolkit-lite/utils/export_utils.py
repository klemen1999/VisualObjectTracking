...
def print_summary(output_dict):
    ...
    print('  Average init speed: %.2f FPS' % output_dict['average_speed_init'])
    print('  Average track speed: %.2f FPS' % output_dict['average_speed_track'])
    print('------------------------------------')
...
def export_measures(workspace_path: str, dataset: Dataset, tracker: Tracker, overlaps: list, failures: list, times: list, times_init:list, times_track:list):
    
    # create per-sequence output structure
    speed = len(dataset.sequences) * [0]
    speed_init = len(dataset.sequences) * [0]
    speed_track = len(dataset.sequences) * [0]
    results = len(dataset.sequences) * [0]
    for i, sequence in enumerate(dataset.sequences):
        speed_fps = 1.0 / times[i]
        speed_fps_init = 1.0 / times_init[i]
        speed_fps_track = 1.0 / times_track[i]
        results[i] = {'sequence_name': sequence.name, 'sequence_length': sequence.length, \
            ...
            "speed_init": speed_fps_init, "speed_track": speed_fps_track}
        speed[i] = speed_fps
        speed_init[i] = speed_fps_init
        speed_track[i] = speed_fps_track

    # average measures
    average_overlap = sum(overlaps) / len(dataset.sequences)
    total_failures = sum(failures)
    average_speed = sum(speed) / len(dataset.sequences)
    average_speed_init = sum(speed_init) / len(dataset.sequences)
    average_speed_track = sum(speed_track) / len(dataset.sequences)
    ...
    output = {'tracker_name': tracker.name(), 'results': results, 'average_overlap': average_overlap, \
        ...
        "average_speed_init": average_speed_init, "average_speed_track":average_speed_track}
