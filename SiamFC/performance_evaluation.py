...
def evaluate_performance(dataset_path, results_dir):
    ...
    for sequence_name in sequences:
        ...
        curr_thresh = estimate_thresholds([scores], 100)
        curr_pr, curr_re, curr_f = calculate_pr_re_f([sequence], [overlaps],
                    [scores], curr_thresh)
        print('--------------------------')
        print(f"{sequence_name}")
        print('Precision:', curr_pr)
        print('Recall:', curr_re)
        print('F-score:', curr_f)
        print('--------------------------')
